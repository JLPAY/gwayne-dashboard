...
@Component({
  ...
  styleUrls: ['./kube-pod.component.scss']
})
...
export class KubePodComponent extends KubernetesNamespacedResource implements OnInit, OnDestroy {
  ...
  diagnoseAllPods() {
    ...
    // 如果命名空间为空或未选择，则不传递 namespace 参数，诊断所有命名空间的 Pod
    const namespace = this.namespace && this.namespace.trim() !== '' ? this.namespace : undefined;

    console.log('开始诊断所有 Pod:', { 
      cluster: this.cluster, 
      namespace: namespace || '所有命名空间'
    });

    // 使用通用诊断接口，诊断 Pod
    // 如果指定了命名空间，则诊断该命名空间的 Pod；否则诊断所有命名空间的 Pod
    ...
    this.k8sgptService.diagnose(diagnoseRequest).subscribe(
      ...
      error => {
        ...
        // 针对特定的后端配置错误提供友好提示
        const namespace = this.namespace && this.namespace.trim() !== '' ? this.namespace : '所有命名空间';
