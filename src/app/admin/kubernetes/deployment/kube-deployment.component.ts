...
export class KubeDeploymentComponent extends KubernetesNamespacedResource implements OnInit, OnDestroy {
  ...
  diagnoseAllDeployments() {
    ...
    // 如果命名空间为空或未选择，则不传递 namespace 参数，诊断所有命名空间的 Deployment
    const namespace = this.namespace && this.namespace.trim() !== '' ? this.namespace : undefined;

    console.log('开始诊断所有 Deployment:', { 
      cluster: this.cluster, 
      namespace: namespace || '所有命名空间'
    });

    // 使用通用诊断接口，诊断 Deployment
    // 如果指定了命名空间，则诊断该命名空间的 Deployment；否则诊断所有命名空间的 Deployment
    ...
    this.k8sgptService.diagnose(diagnoseRequest).subscribe(
      ...
      error => {
        console.error('诊断 Deployment 失败:', error);
        console.error('错误详情:', {
